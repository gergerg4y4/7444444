...
bool g_MenuVisible = true; // always visible until input works
...
VkRenderPass createImGuiRenderPass(){
    ...
    att.format=VK_FORMAT_R8G8B8A8_UNORM; // Quest uses RGBA not BGRA
...
void createImGuiFramebuffers(){
    ...
    for(size_t i=0;i<g_SwapChainImages.size();i++){
        ...
        ci.format=g_SwapChainImages.empty()?VK_FORMAT_R8G8B8A8_UNORM:VK_FORMAT_R8G8B8A8_UNORM;
...
VkResult vkCreateSwapchainKHRReplace(VkDevice dev,const VkSwapchainCreateInfoKHR* pCI,const VkAllocationCallbacks* pA,VkSwapchainKHR* pSC){
    LOGD("vkCreateSwapchainKHR called");
    ...
    if(r==VK_SUCCESS){
        ...
        LOGD("Swapchain format=%d colorSpace=%d", pCI->imageFormat, pCI->imageColorSpace);
        uint32_t cnt;
...
VkResult hooked_vkQueueSubmit(VkQueue,uint32_t,const VkSubmitInfo*,VkFence);
VkResult vkCreateSwapchainKHRReplace(VkDevice,const VkSwapchainCreateInfoKHR*,const VkAllocationCallbacks*,VkSwapchainKHR*);
VkResult vkCreateDeviceReplace(VkPhysicalDevice,const VkDeviceCreateInfo*,const VkAllocationCallbacks*,VkDevice*);
VkResult vkCreateAndroidSurfaceKHRReplace(VkInstance,const VkAndroidSurfaceCreateInfoKHR*,const VkAllocationCallbacks*,VkSurfaceKHR*);

// vkGetDeviceProcAddr hook - intercept Unity's function pointer lookups
PFN_vkVoidFunction (*orig_vkGetDeviceProcAddr)(VkDevice, const char*) = nullptr;
PFN_vkVoidFunction hooked_vkGetDeviceProcAddr(VkDevice device, const char* pName) {
    if(pName) {
        if(strcmp(pName,"vkQueueSubmit")==0)        return (PFN_vkVoidFunction)hooked_vkQueueSubmit;
        if(strcmp(pName,"vkCreateSwapchainKHR")==0) return (PFN_vkVoidFunction)vkCreateSwapchainKHRReplace;
    }
    return orig_vkGetDeviceProcAddr(device, pName);
}

PFN_vkVoidFunction (*orig_vkGetInstanceProcAddr)(VkInstance, const char*) = nullptr;
PFN_vkVoidFunction hooked_vkGetInstanceProcAddr(VkInstance instance, const char* pName) {
    if(pName) {
        if(strcmp(pName,"vkCreateDevice")==0)               return (PFN_vkVoidFunction)vkCreateDeviceReplace;
        if(strcmp(pName,"vkCreateAndroidSurfaceKHR")==0)    return (PFN_vkVoidFunction)vkCreateAndroidSurfaceKHRReplace;
...
void initializeHooks(){
    ...
    // Hook at the proc addr level so we catch Unity even if it already loaded Vulkan
    DobbyHook(DobbySymbolResolver("libvulkan.so","vkGetInstanceProcAddr"),
        (void*)hooked_vkGetInstanceProcAddr,(void**)&orig_vkGetInstanceProcAddr);
    DobbyHook(DobbySymbolResolver("libvulkan.so","vkGetDeviceProcAddr"),
        (void*)hooked_vkGetDeviceProcAddr,(void**)&orig_vkGetDeviceProcAddr);
    LOGD("ProcAddr hooks installed");

    // Also hook directly in case Unity cached pointers before we loaded
    ...
    // Try multiple symbol names for Quest's input system
    const char* keySyms[] = {
        "_ZN7android13InputConsumer19dispatchKeyEventEjPNS_8KeyEventE",
        "_ZN7android13InputConsumer21initializeKeyEventEPNS_8KeyEventEPKNS_12InputMessageE",
        "_ZN7android24InputEventFactoryInterface14createKeyEventEv",
        nullptr
    };
    bool keyHooked = false;
    for(int i = 0; keySyms[i] && !keyHooked; i++){
        void* addr = DobbySymbolResolver("libinput.so", keySyms[i]);
        if(addr){
            DobbyHook(addr,(void*)dispatchKeyEventReplace,(void**)&dispatchKeyEventOrigin);
            LOGD("Key event hook installed: %s", keySyms[i]);
            keyHooked = true;
        }
    }
    if(!keyHooked) LOGD("Key event hook not found - will use OVR polling instead");
